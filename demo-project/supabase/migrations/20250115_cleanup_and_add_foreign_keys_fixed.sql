...
-- Fixed to use correct table names: 'users' instead of 'profiles'
...
-- Step 8: Insert some sample data to test the relationships
-- First, ensure we have at least one user for testing
INSERT INTO users (id, email, full_name, wallet_address, kyc_status, account_status, created_at, updated_at)
VALUES 
    ('fdfbc753-e956-4b6a-8c44-f25f9d428d6d', 'test@example.com', 'Test User', '0x1234567890123456789012345678901234567890', 'pending', 'active', NOW(), NOW())
ON CONFLICT (id) DO UPDATE SET
    full_name = EXCLUDED.full_name,
    wallet_address = EXCLUDED.wallet_address,
    updated_at = NOW();

-- Add a sample asset linked to this user
INSERT INTO assets (id, user_id, name, asset_type, description, current_value, original_value, verification_status, collateralization_status, location, blockchain)
VALUES 
    (gen_random_uuid(), 'fdfbc753-e956-4b6a-8c44-f25f9d428d6d', 'Sample Real Estate Token', 'real_estate', 'Test property for admin demo', 100000.00, 95000.00, 'pending', 'available', 'San Francisco, CA', 'ethereum')
ON CONFLICT DO NOTHING;

-- Add another sample asset
INSERT INTO assets (id, user_id, name, asset_type, description, current_value, original_value, verification_status, collateralization_status, location, blockchain)
VALUES 
    (gen_random_uuid(), 'fdfbc753-e956-4b6a-8c44-f25f9d428d6d', 'Commercial Building Token', 'commercial_real_estate', 'Office building in downtown', 250000.00, 240000.00, 'pending', 'available', 'New York, NY', 'polygon')
ON CONFLICT DO NOTHING; 